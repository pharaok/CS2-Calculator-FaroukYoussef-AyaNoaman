...
string parseBrackets(string expression) {
    ...
    for (int i = 0; i < expression.length(); ++i) {
        ...
        if (start_count && start_count == stop_count) {
            // when the start_ind bracket is finally closed, the expression inside will be returned
...
string parseMultDiv(string expression) {
    /**
     * Determines where the RHS of the multiplication/division ends to ensure correct precedence.
     */
                    ...
                    for (int j = i+1; j < expression.length(); ++j) {
                        ...
                        if (a == ' ') {
                            ...
                            if (end_ind == expression.length() - 1) {
                                ...
                                return compute(to_string(div(stold(part), compute(content))) + expression.substr(end_ind + 1));
                            }
                        } else if (string("(lg").find(a) != string::npos) {
                            switch (a) {
                                case '(': {
                                ...
                                case 'g': {
                                    ...
                                    if (end_ind == expression.length() - 1) {
                                        ...
                                        return compute(to_string(div(stold(part), (long double)gcd(stoll(first_item), stoll(second_item)))) + expression.substr(end_ind + 1));
                                    }
                                }
                                ...
                                case 'l': {
                                    ...
                                    string second_item = to_string(compute(content.substr(comma_ind + 1, end_ind - expression.find(',') - 1)));
                                    if (end_ind == expression.length() - 1) {
                                        return div(stold(part), (long double)lcm(stoll(first_item), stoll(second_item)));
                                    } else {
                                        return compute(to_string(div(stold(part), (long double)lcm(stoll(first_item), stoll(second_item)))) + expression.substr(end_ind + 1));
                                    }
                                }
                            }
                        }
